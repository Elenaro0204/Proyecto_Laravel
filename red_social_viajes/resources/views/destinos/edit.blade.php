...
<div class="max-w-3xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <h1 class="text-2xl font-semibold mb-6">Editar destino</h1>
    ...
    <form action="{{ route('destinos.update', $destino) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="pais" id="pais" value="{{ old('pais', $destino->pais) }}" required
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" />
        </div>
        ...
        <div>
            ...
            <textarea name="descripcion" id="descripcion" rows="4"
                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('descripcion', $destino->descripcion) }}</textarea>
        </div>
