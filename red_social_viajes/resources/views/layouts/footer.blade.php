<footer class="bg-gray-900 border-t border-gray-700">
    <div class="max-w-7xl mx-auto px-6 py-8 grid grid-cols-1 md:grid-cols-3 gap-8 text-gray-300">
        <!-- Logo y Descripción -->
        <div>
            <h2 class="text-xl font-semibold text-indigo-400 mb-2">GoTogether</h2>
        ...
        <!-- Navegación -->
        <div>
            <h3 class="text-lg font-semibold mb-3 text-indigo-400">Navegación</h3>
            <ul class="space-y-2">
                <li><a href="{{ route('index') }}" class="hover:text-indigo-600 transition">Inicio</a></li>

                @if(auth()->user() && !auth()->user()->isAdmin())
                    <li><a href="route('viajes.index')" class="hover:text-indigo-600 transition">
                        Viajes
                        </a>
                    </li>
                @endif
                ...
                @if(auth()->user() && auth()->user()->isAdmin())
                    <li><a href="route('admin.viajes.index')"  class="hover:text-indigo-600 transition">
                        Viajes
                        </a>
                    </li>
                @endif

                <li><a href="{{ route('destinos.index') }}" class="hover:text-indigo-600 transition">Destinos</a></li>

                @if(auth()->user() && auth()->user()->isAdmin())
                    <li><ahref="route('admin.index')" class="hover:text-indigo-600 transition">
                        Admin
                        </a>
                    </li>
                @endif

                <li><a href="{{ route('profile.index') }}" class="hover:text-indigo-600 transition">Perfil</a></li>
            </ul>
        </div>

        <!-- Contacto o Redes sociales -->
        <div>
            <h3 class="text-lg font-semibold mb-3 text-indigo-400">Contacto</h3>
            <p class="text-sm mb-2">Correo: soporte@gotogether.com</p>
            <p class="text-sm mb-4">Teléfono: +34 600 123 456</p>
            <div class="flex space-x-4">
                <a href="#" aria-label="Facebook" class="text-gray-400 hover:text-indigo-600 transition">
                ...
                <a href="#" aria-label="Instagram" class="text-gray-400 hover:text-indigo-600 transition">
